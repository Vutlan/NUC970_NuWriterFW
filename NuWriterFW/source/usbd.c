...
/* maximum string descriptor index */
#define MAX_STR_DSCPT       2
...
void ledDisplayInit()
{
	// gpioa.10 = err_led; gpioa.11 = act_led
    outpw(REG_MFP_GPA_H, inpw(REG_MFP_GPA_H) & (~0x0000000f));	// GPA10 select gpio function
    outpw(REG_GPIOA_DIR, inpw(REG_GPIOA_DIR) & ~(1<<10));
    sysDelay(2);
   	ledpolarity = (UINT8)(inpw(REG_GPIOA_DATAIN) >> 10) & 1;
    outpw(REG_GPIOA_DIR, inpw(REG_GPIOA_DIR) | (1<<10) | (1<<11)); // set GPA10 as out
    //MSG_DEBUG("ledpolarity=%d\n", ledpolarity);
}
...
void ledDisplay(UINT8 u8parm)
{
	if (u8parm != 0)
		u8parm = 1;
    if (u8parm ^ (ledpolarity &1)) {
        outpw(REG_GPIOA_DATAOUT, (inpw(REG_GPIOA_DATAOUT) | (1<<10)) & ~(1<<11));	// set GPA10 to high
    } else {
        outpw(REG_GPIOA_DATAOUT, (inpw(REG_GPIOA_DATAOUT) & ~(1<<10)) | (1<<11));	// set GPA10 to low
    }
    //MSG_DEBUG("REG_GPIOA_DATAOUT=0x%x\n", inpw(REG_GPIOA_DATAOUT));
}

extern UINT8 udcFlashInit(void);

/*!<USB Device Descriptor */
ALIGN_ATTR(4) static UINT16 _DeviceDescriptor[10] = {
    0x0112, 0x0200, 0x0000, 0x4000, 0x0416, 0x5963, 0x0100,
#ifdef WINDOWS_HOST
	0x0201,		/* NUC970 IBR 20151008.1735 */
#else
	0x0201,
#endif
	0x0100, 0x0000
...
ALIGN_ATTR(4) static UINT16 _QualifierDescriptor[6] = {
    0x060a, 0x0200, 0x0000, 0x4000, 0x0001, 0x0000
};

#ifndef DISABLE_USB_HIGH_SPEED
ALIGN_ATTR(4) static UINT16 _ConfigurationBlock[16] = {
    0x0209, 0x0020, 0x0101, 0xC000, 0x0932, 0x0004, 0x0200, 0x0000, 0x0000, 0x0507,
    0x0281, 0x0200, 0x0701, 0x0205, 0x0002, 0x0102
};
#endif

ALIGN_ATTR(4) static UINT16 _ConfigurationBlockFull[16] = {
    0x0209, 0x0020, 0x0101, 0xc000, 0x0932, 0x0004, 0x0200, 0x0000, 0x0000, 0x0507,
    0x0281, 0x0040, 0x0701, 0x0205, 0x4002, 0x0100
};

ALIGN_ATTR(4) static UINT16 _OSConfigurationBlock[16] = {
    0x0709, 0x0020, 0x0101, 0xc000, 0x0932, 0x0004, 0x0200, 0x0000, 0x0000, 0x0507,
    0x0281, 0x0040, 0x0701, 0x0205, 0x4002, 0x0100
};

ALIGN_ATTR(4) static UINT16 _StringDescriptor0[2] = {
    0x0304, 0x0409
};

ALIGN_ATTR(4) static UINT16 _StringDescriptor1[11] = {
    0x0316, 0x0055, 0x0053, 0x0042, 0x0020, 0x0044, 0x0065, 0x0076, 0x69, 0x0063, 0x0065
};

ALIGN_ATTR(4) static UINT16 _StringDescriptor2[26] = {
    ...
    if ((GET_DEV_Flag == 1) || (GET_QUL_Flag == 1) || (GET_CFG_Flag == 1) ||
        ...
        if (_usbd_remlen_flag == 0) {
            if (GET_DEV_Flag == 1)
                ...
                //MSG_DEBUG("I%dL%dD%d\t0x%x\n",_usb_cmd_pkt.wValue & 0xff, _usb_cmd_pkt.wLength, (UINT16)*ptr, (UINT32)ptr);
                if (_usb_cmd_pkt.wLength > (UINT16)*ptr)
                	_usb_cmd_pkt.wLength = (UINT16)*ptr;  // actual descriptor length
...
void usbd_control_packet()
{
    ...
    case USBR_GET_DESCRIPTOR:
        ...
        case USB_DT_STRING:
            ...
            /* unsupported index / OS string descriptor (0xee) - send stall  */
            if (((_usb_cmd_pkt.wValue & 0xff) > MAX_STR_DSCPT))
            	ReqErr =1;
            else
            	GET_STR_Flag = 1;
            break;
...
void udcInit(void)
{
...
#ifdef RESET_USB
    /* disable PHY (reset value)  */
    MSG_DEBUG("USB PHY disable\n");
    /* outpw(REG_USBD_PHY_CTL, 0x0420); */

    /* usbd  reset */
    outpw(REG_AHBIPRST, inpw(REG_AHBIPRST) | (1<<16));
    /* Pause for USB unplug detection */
    sysDelay(399);   // 4 sec
    outpw(REG_AHBIPRST, inpw(REG_AHBIPRST) & ~(1<<16));
    sysDelay(1);
#endif
    ...
    usbdHighSpeedInit();
#else
    _usbd_speedset = 1;		// default at full speed mode
    usbdFullSpeedInit();
#endif
