...
typedef struct __attribute__((__packed__)) _info {
...
static UINT32 timer_event_no = -1;
static void IndicationOff(void)
{
	sysClearTimerEvent(TIMER0, timer_event_no);
	timer_event_no = -1;
	ledDisplay(0);
	//MSG_DEBUG("%s\n", __FUNCTION__);
}
...
void IndicationCallback(void)
{
	toggle ^= 1;
	ledDisplay(toggle);
	//MSG_DEBUG("%d\n",toggle);
}
static void IndicationBlink(UINT32 period)
{
	if (timer_event_no == -1) {
		timer_event_no = sysSetTimerEvent(TIMER0, period, (PVOID) IndicationCallback);
		//MSG_DEBUG("%s event=%d\n", __FUNCTION__, timer_event_no);
	}
}
